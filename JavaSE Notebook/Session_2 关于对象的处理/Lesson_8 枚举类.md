...
在jdk1.5之前是需要我们自己创建枚举类的，在jdk1.5之后提供了enum帮我们创建枚举类，不过原理都是相同的。枚举类就是有固定对象的类，其中类的属性用private和final修饰，构造方法私有化。如果类中有属性肯定要有构造方法为其属性赋值，因为属性使用private和final修饰的不能通过set进行赋值，枚举类中的对象用public、static和final修饰。<br/>
